//Basic Traffic light Operation and Interruptions 2
//Author: Tanvir Hossain
//Version: 08th October, 2018
...
//The delay function, called by main.
...
void PORT1_IRQHandler(void){
  FallingEdges = FallingEdges + 1;
	...
	//code for the car switch
		
  } else if ((P1IFG & 0x10) != 0) {
		
		P1IFG &= ~0x10;		// clear flag4, acknowledge
		if(color == RED && timeRemaining > minredTime ){
		
			color = RED;
			P2OUT &= 0x00;
			P2OUT ^= RED;	
			timeRemaining = minredTime;
...
int main(void){
  ...
  while(1){
		...
		//Using a variable timeRemaining to make life easier
		while(timeRemaining > 0){
			delay1m(1000);
			timeRemaining--;
		}
		
		// Changing the state colour depending on the previous color
		if(color == GREEN){
			color = YELLOW;
			P2OUT &= 0x00;
			P2OUT ^= GREEN;
